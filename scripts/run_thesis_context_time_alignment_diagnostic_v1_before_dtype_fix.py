...
def resolve(df: pd.DataFrame, bridge: dict[str, Any]) -> pd.DataFrame:
    ...
    if "coin_id" in out.columns:
        ...
        out["canonical_coin_id"] = cid.map(lambda x: bridge["id_map"].get(int(x), pd.NA) if pd.notna(x) else pd.NA)

    if "pair_address" in out.columns:
        unresolved = out["canonical_coin_id"].isna()
        pair = out["pair_address"].map(norm)
        mapped = pair.map(lambda x: bridge["pair_map"].get(x, pd.NA))
        out.loc[unresolved & mapped.notna(), "canonical_coin_id"] = mapped.loc[unresolved & mapped.notna()]
    ...
    out["event_time_utc"] = pd.to_datetime(out["timestamp"], errors="coerce", utc=True) if "timestamp" in out.columns else pd.NaT
    return out
...
def market_future_diagnostics(events: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    m_by_coin = {
        int(cid): grp["event_time_utc"].astype("int64").to_numpy()
