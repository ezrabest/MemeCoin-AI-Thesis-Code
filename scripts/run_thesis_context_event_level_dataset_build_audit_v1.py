...
def build_signal_events(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    events = g.agg(
        ...
        candidate_event_time_utc=("event_time_utc", "max"),
...
def attach_market_and_labels(events: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, r in out.iterrows():
        ...
        times = m["event_time_utc"].astype("int64").to_numpy()
        prices = m["market_price"].to_numpy(dtype=float)

        ev_ns = pd.Timestamp(ev_time).value
...
def attach_context_counts(events: pd.DataFrame, context: pd.DataFrame, prefix: str, numeric_cols: list[str]) -> pd.DataFrame:
    ...
    for idx, r in out.iterrows():
        ...
        cdf = ctx_by_coin[cid]
        times = cdf["event_time_utc"].astype("int64").to_numpy()
        ev_ns = pd.Timestamp(ev_time).value

        for h in CONTEXT_WINDOWS_HOURS:
            start_ns = (pd.Timestamp(ev_time) - pd.Timedelta(hours=h)).value
...
def main() -> None:
    summary = {
        ...
        "sl_ratio": SL_RATIO,
        "notes": [
            "candidate event timestamp is the last signal timestamp inside the event bucket",
    ...
    lines.append(f"- Candidate timestamp: last signal timestamp inside event bucket")
